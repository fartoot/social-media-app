...
def vote(vote: schemas.Vote ,db: Session = Depends(database.get_db), user_id: int = Depends(oauth2.verify_access_token)):
   ...
   print(vote_query.first())
   if not vote_query.first():
      db.add(models.Vote(post_id=vote.post_id,user_id=user_id))
      db.commit()
      return Response(status_code=200, content=f"User {user_id} add vote to post {vote.post_id}")
   else:
        vote_query.delete()
        db.commit()
        return Response(status_code=200, content=f"User {user_id} delete vote to post {vote.post_id}")