...
@pytest.mark.parametrize("content,published", [
    ("test content 1", True),
    ("test content 2", False),
    ("test content 3", True),
])
def test_create_post(authorized_client,created_user, content, published):
    res = authorized_client.post("/posts/", json={"content":content, "published": published})
...
def test_unauthorized_user_create_post(client):
    res = client.post("/posts/",json={"content":"test content"})
    assert res.status_code == 401
